...
contextBridge.exposeInMainWorld('aetherisBridge', {
  ...
  onPatchAvailable: (cb) => ipcRenderer.on('patch-available-event', (_e, info) => cb(info)),
  getPatchStatus: () => ipcRenderer.invoke('patch-status'),
  checkForPatch: () => ipcRenderer.invoke('patch-check'),
  installPatch: (manifest) => ipcRenderer.invoke('patch-install', manifest),
  restartForPatch: () => ipcRenderer.invoke('patch-restart'),
